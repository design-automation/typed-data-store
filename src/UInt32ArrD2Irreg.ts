...
export class Uint32ArrD2Irreg {
    // static
    private static BUFF_START_SIZE = 1024 * 8;
    private static BUFF_STEP_SIZE = 1024 * 8;
    // index array
    private _idx_view: Uint32Array = null;
    // len array
    private _len_view: Uint16Array = null;
    // data array
    ...
    private _extendIdxLenBuff(ints_to_add: number): void {
        if (this._num_arrs + ints_to_add < this._idx_view.length) { return; }
        // console.log('XXX EXTEND IDX LEN BUFF XXX');
        const new_num_ints: number = this._idx_view.length +
        ...
        new_idx_view.set(this._idx_view);
        this._idx_view = new_idx_view;
        // len
        const new_len_view: Uint16Array = new Uint16Array(
            new ArrayBuffer(new_num_ints * Uint16Array.BYTES_PER_ELEMENT)
        );
        new_len_view.set(this._len_view);
        this._len_view = new_len_view;
    }
    /**
    ...
    private _extendDataBuff(ints_to_add: number): void {
        if (this._idx_view[this._num_arrs] + ints_to_add <= this._data_view.length) { return; }
     ...
     */
    private _calcReqSpace(len: number): number {
        return len <= this._padding ?
            this._padding :
            Math.ceil(len / this._padding) * this._padding;
    }
    // --------------------------------------------------------------------------------------------
    // --------------------------------------------------------------------------------------------
    ...
    public setArr(idx0: number, arr: number[]): number {
        ...
        if (idx0 < this._num_arrs) { // overwrite
            ...
            // make enough space
            if (arr.length > space) {
                const req_space: number = this._calcReqSpace(arr.length);
            ...
            // add empty arrs
            const num_arrs_to_add: number = idx0 - this._num_arrs + 1;
            ...
            const num_ints_to_add: number = ((num_arrs_to_add - 1) * this._padding) + req_space;
            this._extendDataBuff(num_ints_to_add);
            // update len and idx for each new empty arr
            for (let i = 0; i < num_arrs_to_add; i++) {
                const next_idx0: number = this._num_arrs + i;
                this._len_view[next_idx0] = 0; // undefined
                this._idx_view[next_idx0] = end_idx2 + (i * this._padding);
            }
            // update _num_arrs
            this._num_arrs += num_arrs_to_add;
            // get the new idx2 of the last arr
            const new_end_idx2: number = this._idx_view[this._num_arrs - 1];
            // set the data for the last arr
            this._data_view.set(arr, new_end_idx2);
            // update _data_view length
            this._idx_view[this._num_arrs] = end_idx2 + num_ints_to_add;
        }
        // update len
    ...
    private _setArrUndef(idx0: number): number {
        ...
        // create space for intermediate arrs
        // special case
        ...
        // extend
        this._extendIdxLenBuff(emp_arrs_to_add);
        // update idx arr
    ...
    public insArr(idx0: number, arr: number[]): number {
        ...
        this._extendDataBuff(arr.length  + this._padding);
        // shift data
     ...
     */
    public toNestedArr(limit?: number, end?: boolean): number[][] {
        limit = limit === undefined ? this._num_arrs : limit;
        const arrs: number[][] = [];
        // get the whole thing
        if (this._num_arrs < limit) {
        ...
        // get the end
        for (let next_idx0 = this._num_arrs - limit; next_idx0 < this._num_arrs; next_idx0++) {
            arrs.push(this.getArr(next_idx0));
        }
        return arrs;
    }
    ...
    public toStr(limit: number): string {
        ...
        const start: number[][] = this.toNestedArr(half_limit, false);
        const end: number[][] = this.toNestedArr(half_limit, true);
    ...
    public toDebugStr(limit?: number): string {
        ...
        if (this._num_arrs < limit) {
            // data
            data_arr_str = '[' + this._data_view.subarray(0, this._idx_view[this._num_arrs]) + ']';
            // idx
            idx_arr_str = '[' + this._idx_view.subarray(0, this._num_arrs + 1) + ']';
            // len
            len_arr_str = '[' + this._len_view.subarray(0, this._num_arrs + 1) + ']';
        } else {
            const half_limit: number = Math.ceil(limit / 2);
            const end_start: number = this._num_arrs - half_limit;
            // data
            data_arr_str =  '[' +
                this._data_view.subarray(0, this._idx_view[half_limit]) + ' ... '  +
                this._data_view.subarray(this._idx_view[end_start], this._idx_view[this._num_arrs]) +
            ']';
            // idx
            idx_arr_str = '[' +
                this._idx_view.subarray(0, half_limit) + ' ... '
                + this._idx_view.subarray(end_start, this._num_arrs + 1) +
            ']' ;
            // len
            len_arr_str = '[' +
                this._len_view.subarray(0, half_limit) + ' ... '  +
                this._len_view.subarray(end_start, this._num_arrs + 1) +
            ']';
        }
        // return the str
        return [
            ...
            'Idxlen buff size = \t' + this._idx_view.length,
